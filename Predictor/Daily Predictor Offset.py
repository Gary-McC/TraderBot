    ...
    while cur_time < end_time:
        ...
        for i in range(0,len(dat)):
            ...
            while (track != clean_time):
                df=df.append({'Time':track.strftime('%H:%M:%S'),'Low':None,'High':None,'Open':None,'Close':None,'Volume':None},ignore_index=True)
                track=track-five_min
            df=df.append({'Time':track.strftime('%H:%M:%S'),'Low':rest[0],'High':rest[1],'Open':rest[2],'Close':rest[3],'Volume':rest[4]},ignore_index=True)
            track=track-five_min
...
for i in range(seq_len, len(train_data)):
  ...
  y_train.append(train_target[i]) #Value of 4th column (Close Price) of df-row 128+1
...
callback = tf.keras.callbacks.ModelCheckpoint('C:/Users/Annoy/Desktop/Spyder/Useful Code Snippets/Models/PredictiveModels/'+Modelname+'_'+time+'.hdf5', 
                                              monitor='val_loss', 
                                              save_best_only=True, 
                                              verbose=1)
...
model = tf.keras.models.load_model('C:/Users/Annoy/Desktop/Spyder/Useful Code Snippets/Models/PredictiveModels/'+Modelname+'_'+time+'.hdf5',
                                   custom_objects={'Time2Vector': Time2Vector, 
                                                   'SingleAttention': SingleAttention,
                                                   'MultiAttention': MultiAttention,
                                                   'TransformerEncoder': TransformerEncoder})
...
ax11.plot(train_data[:, 3], label='Coin Closing Returns')
ax11.plot(np.arange(seq_len+offset, train_pred.shape[0]+seq_len+offset), train_pred, linewidth=3, label='Predicted Coin Closing Returns')
ax11.set_title("Training Data", fontsize=18)
ax11.set_xlabel('Time')
ax11.set_ylabel('Coin Closing Returns')
ax11.legend(loc="best", fontsize=12)

#Plot validation data results
ax21 = fig.add_subplot(312)
ax21.plot(val_data[:, 3], label='Coin Closing Returns')
ax21.plot(np.arange(seq_len+offset, val_pred.shape[0]+seq_len+offset), val_pred, linewidth=3, label='Predicted Coin Closing Returns')
ax21.set_title("Validation Data", fontsize=18)
ax21.set_xlabel('Time')
ax21.set_ylabel('Coin Closing Returns')
ax21.legend(loc="best", fontsize=12)

#Plot test data results
ax31 = fig.add_subplot(313)
ax31.plot(test_data[:, 3], label='Coin Closing Returns')
ax31.plot(np.arange(seq_len+offset, test_pred.shape[0]+seq_len+offset), test_pred, linewidth=3, label='Predicted Coin Closing Returns')
